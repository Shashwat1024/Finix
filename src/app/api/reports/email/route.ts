...
<title>Finix Daily — ${data.month}</title>
</head>
...
<table width="620" cellpadding="0" cellspacing="0" style="max-width:620px;width:100%;">
  ...
  <tr><td style="padding:16px 0;text-align:center;">
    <p style="margin:0;font-size:10px;color:#334155;font-family:system-ui;">
      Generated by Finix AI · ${date} · This is not financial advice.
    </p>
...
Return ONLY valid JSON with this exact structure:
{
  ...
  const fromAddress = process.env.RESEND_FROM ?? 'Finix <onboarding@resend.dev>'

  try {
    const { error } = await resend.emails.send({
      from: fromAddress,
      to: user.email!,
      subject: `The Finix Daily — ${monthLabel}`,
      html,
    })
